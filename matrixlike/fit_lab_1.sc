...
object fit_lab_1 {
  ...
  // theta (0,0) applied to X with intercept
	...
	// cost formula
	diff(X, y)                                //> res0: List[Double] = List(-1.0, -2.0, -6.0, -12.0)
	dotPow(diff(X, y), 2)                     //> res1: List[Double] = List(1.0, 4.0, 36.0, 144.0)
	dotPow(diff(X, y), 2).reduceRight(_+_)    //> res2: Double = 185.0
	dotPow(diff(X, y), 2).reduceRight(_+_)/2  //> res3: Double = 92.5
	...
	cost(X, y)                                //> res4: Double = 92.5
	
	cost(X3, y)                               //> res5: Double = 178.5
	...
	val X10 = matxProd(X2, theta8).flatten    //> X10  : List[Double] = List(1.5999999999999996, 5.8999999999999995, 10.2, 14
                                                  //| .5)
	cost(X10, y)                              //> res12: Double = 3.7299999999999986
}