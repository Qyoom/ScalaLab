...
object Authorize_lab_3 {
	...
	val relevantReqPerms = for (rp <- requiredPerms) yield {
	    rp match {
	        case RequiredPermission(id, Some(projectId), "project/update", "ORG_ADMIN", rule) if (projectId == 13) => {
	            rp
	        }
	    }
	}
